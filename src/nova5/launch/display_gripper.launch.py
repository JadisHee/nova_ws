...
def generate_launch_description():
    ...
    default_urdf_path = os.path.join(urdf_package_path,'urdf/gripper','RG52_050.xacro')
    default_rviz_path = os.path.join(urdf_package_path,'rviz','display_nova5.rviz')


    # 声明一个urdf目录的参数，以便修改
    action_declare_arg_mode_path = launch.actions.DeclareLaunchArgument(
        name='model',default_value=str(default_urdf_path),description='launch the urdf path'
    )

    # 通过文件路径，获取内容，转换成参数值对象，以供传入 robot_state_publisher
