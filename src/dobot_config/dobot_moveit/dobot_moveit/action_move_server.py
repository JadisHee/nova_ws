...
class FollowJointTrajectoryServer(Node):
    ...
    def __init__(self):
        ...
        self._action_server = ActionServer(self,FollowJointTrajectory,f'/{name}_arm_controller/follow_joint_trajectory',self.execute_callback)
        ...
        for i, point in enumerate(trajectory.points):
            ...
            self.get_logger().info(
                "Point {}: Positions: {}, Velocities: {}, Accelerations: {}, TimeFromStart: {}".format(
                    i, joint, point.velocities, point.accelerations, float(point.time_from_start.sec + point.time_from_start.nanosec/1000000000)
                )
            )
        for ii in Positions:
